...
public class UserDaoBean extends Dao implements UserDao {
	...
	private ChangeResult localCreateUserInfo(CreateUserInput request, Id userId) throws DaoException {
		...
		Object[] parameters = DaoHelper.generateArray(userId.getId(), request.getUser().getUserInfo().getFirstName(),
				request.getUser().getUserInfo().getLastName(), request.getUser().getUserInfo().getBirthDate(), request
						.getUser().getUserInfo().getGender().toString(), request.getUser().getUserInfo().getEmail(),
				request.getUser().getUserInfo().getPhone());
		return super.create(sql, parameters);
	}

	private ChangeResult localCreateUserAddress(CreateUserInput request, Id userId) throws DaoException {
		if (request.getUser().getUserAddress() != null) {
			...
			Object[] parameters = DaoHelper.generateArray(userId.getId(), request.getUser().getUserAddress()
					.getAddress(), request.getUser().getUserAddress().getPostalCode(), request.getUser()
					.getUserAddress().getMunicipality(), request.getUser().getUserAddress().getRegion(), request
					.getUser().getUserAddress().getCountry());
			return super.create(sql, parameters);
		} else {
	...
	private ChangeResult localCreateUserPassword(CreateUserInput request, Id userId) throws DaoException {
		...
		Object[] parameters = DaoHelper.generateArray(userId.getId(), request.getUserPassword().getPassword(), request
	...
	public ReadUserPasswordOutput readUserPassword(ReadUserPasswordInput request) throws DaoException {
		...
		Id userId = localReadUserId(request.getUserName());
	...
	public UpdateUserOutput updateUser(UpdateUserInput request) throws DaoException {
		...
		if (request.getUser().getUserId() != null) {
			Id userId = localReadUserId(request.getUser().getUserId().getUserName());
	...
	private ChangeResult localUpdateUserInfo(UpdateUserInput request, Id userId) throws DaoException {
		if (request.getUser().getUserInfo() != null) {
			...
			Object[] parameters = DaoHelper.generateArray(request.getUser().getUserInfo().getFirstName(), request
					...
					.getUser().getUserInfo().getGender().toString(), request.getUser().getUserInfo().getEmail(),
					request.getUser().getUserInfo().getPhone(), userId.getId());
	...
	private ChangeResult localUpdateUserStatus(UpdateUserInput request, Id userId) throws DaoException {
	...
	private ChangeResult localUpdateUserPassword(UpdateUserInput request, Id userId) throws DaoException {
		if (request.getUserPassword() != null) {
			SQL sql = new SQL.Builder().doUpdate(USER_PASSWORD_TABLE).setColumn(PASSWORD_COLUMN_NAME)
					.where(USER_ID_COLUMN_NAME).build();
			Object[] parameters = DaoHelper.generateArray(request.getUserPassword().getPassword(), userId.getId());
			return super.update(sql, parameters);
		} else {
			return new ChangeResult(0);
		}
	...
	private ChangeResult localUpdateUserAddress(UpdateUserInput request, Id userId) throws DaoException {
		if (request.getUser().getUserAddress() != null) {
			SQL sql = new SQL.Builder()
